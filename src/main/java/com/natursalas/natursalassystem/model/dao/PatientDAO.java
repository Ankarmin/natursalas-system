...
public class PatientDAO implements IPatientDAO {
    ...
    public boolean addPatient(PatientDTO newPatient) {
        String query = "INSERT INTO patient (DNI, firstName, lastName, phoneNumber, dateOfConsultation, dateOfBirth, district, idLocation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public boolean updatePatient(PatientDTO updatedPatient) {
        String query = "UPDATE patient SET firstName = ?, lastName = ?, phoneNumber = ?, dateOfConsultation = ?, dateOfBirth = ?, district = ?, idLocation = ? WHERE DNI = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            ...
            stmt.setTimestamp(5, updatedPatient.getDateOfEntry());
